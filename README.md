...
- BigDataViewer XML/HDF5 datasets are detected from `SpimData` XML and parsed as metadata-only `bdv` using `ViewSetup` size/channel and timepoint declarations; HDF5 dataset inspection, actual pixel type detection, pixel reads, multi-resolution metadata, labels/ROIs, colors, and physical calibration are not yet implemented.
...
- CellH5 `.ch5` HDF5 files are detected from CellH5 group markers and parsed as metadata-only from the first rank-5 CTZYX dataspace; full HDF5 traversal, multi-series plate/well/site mapping, segmentation/ROI/classification metadata, chunk/deflate pixel reads, and LUTs are not yet implemented.
...
- Olympus cellSens `.vsi` files with TIFF-readable embedded planes are delegated through the TIFF path and reported as `cellsens`; external `.ets` pyramids, proprietary VSI tag-tree metadata, channel/Z/T assembly, tile offsets, and acquisition metadata are not yet handled.
...
- Olympus FV1000 OIF datasets are parsed from the `.oif` `ProfileSaveInfo` companion plus neighboring `.pty` files and delegated through TIFF planes as `fv1000`; OIB containers, ROI/LUT metadata, previews, physical calibration, and detailed acquisition metadata are not yet handled.
...
- Bitplane Imaris HDF `.ims` files are detected from the HDF5 signature and parsed as metadata-only `imarishdf` from compact Imaris attributes such as `ImageSizeX/Y/Z`, `FileTimePoints`, and channel labels; HDF5 dataset traversal, pixel type detection beyond the current uint8 default, LZ4/zlib/deflate decompression, multiresolution series, physical sizes, timestamps, and pixel reads are not yet implemented.
...
- Tile JPEG path requests reuse the JPEG header parser and report metadata-only `tilejpeg`; TurboJPEG-backed tile decoding, entropy decoding, and pixel reads are not yet implemented.
...
- Leica LOF files are detected from the `LMS_Object_File` header and parsed as metadata-only `lof` from the embedded LASX XML section; raw memory-block pixels, tile offsets, LUTs, physical sizes, ROIs, timestamps, and instrument metadata are not yet implemented.
...
- Nikon ND2 files are detected from Nikon magic bytes and common embedded XML/text metadata keys are parsed as metadata-only `nd2`; chunk maps, JPEG-2000/zlib/raw pixel decoding, multi-position series, ROIs, LUTs, and full Nikon metadata are not yet implemented.
...
- Olympus OIR files are detected from `OLYMPUSRAWFORMAT` and interspersed XML blocks are parsed as metadata-only `oir`; pixel block assembly, external `.oir` companion chunks, LUTs, physical sizes, timestamps, and instrument metadata are not yet surfaced.
...
- Legacy SlideBook `.sld/.spl` files are detected from native endian/magic headers and parsed as metadata-only `slidebook` from fixed-size `i`/`u` metadata blocks; raw pixel block discovery, spool metadata, SlideBook 7 `.sldy/.sldyz`, YAML metadata, Zstd/Numpy planes, annotations, positions, and channel names are not yet implemented.
- SlideBook 7 `.sldy/.sldyz` datasets are parsed as metadata-only `slidebook7` by resolving the companion `.dir/* .imgdir/ImageRecord.yaml`; multi-capture selection beyond the first image group, `.npy/.npyz` pixel reads, Zstd decompression, masks, annotations, stage positions, and channel/lens metadata are not yet implemented.
...
- TissueFAXS `.tfcyto` SQLite databases, or `.aqproj` projects resolved to the first `Slide */*.tfcyto` database, are parsed as metadata-only from `region`, `fovs`, `images`, and `channels` tables; tile JPEG/JPEG-XR/raw decompression, stitching, TMA subregions, correction images, pyramid resolutions, physical calibration, and full OME metadata are not yet implemented.
...
- Zeiss ZVI files are detected from OLE/CFB image streams and parsed as metadata-only `zeisszvi` from embedded AxioVision image dimensions; POI stream metadata, JPEG/zlib decompression, BGR channel correction, tiled/ROI metadata, and pixel reads are not yet implemented.
