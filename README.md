...
- Zeiss LSM TIFF files identified by the private `TIF_CZ_LSMINFO` tag, decoded through the TIFF pixel path with paired thumbnail IFDs skipped and basic Z/C/T dimensions and scan-type dimension order parsed from the LSM info block; channel names/colors, LUTs, timestamps, ROIs, and MDB multi-file grouping are not yet parsed.
