...
- Olympus OIR files are detected from `OLYMPUSRAWFORMAT`, interspersed XML blocks provide dimensions, and simple one-full-raw-block-per-plane datasets are read directly; multi-block/tiled plane assembly, external `.oir` companion chunks, LUTs, physical sizes, timestamps, and instrument metadata are not yet surfaced.
