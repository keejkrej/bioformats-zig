...
- InCell 1000/2000 `.xdce`/`.xml` companion datasets delegated through referenced or synthesized TIFF plane names and reported as `incell`, including Java-style default-series missing Z-plane duplication and zero-filled missing channels; raw `.im` planes, full HCS plate/well/field multi-series reads, and acquisition metadata are not yet surfaced.
...
- Leica LIF files are detected from the LAS AF memory/XML header and decoded from uncompressed memory blocks for scalar integer/floating-point planes, including simple row padding, BGR-to-RGB correction for RGB samples, Bio-Formats-style per-series core metadata, `MemoryBlockID` lookup, and path-based JSON-RPC `series` plane reads; tiled fields, physical sizes, LUTs, ROIs, timestamps, and instrument metadata are not yet implemented.
