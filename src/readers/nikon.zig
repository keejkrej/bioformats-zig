...
pub fn matches(data: []const u8) bool {
    if (tiff.firstIfdContainsTag(data, tiff_ep_standard_tag)) return true;
    const make = tiff.firstIfdAsciiTag(data, make_tag) orelse return false;
    return std.mem.indexOf(u8, make, "Nikon") != null;
}

pub fn readMetadata(data: []const u8) bio.ReaderError!bio.Metadata {
    var metadata = try tiff.readMetadata(data);
    metadata.format = "nikon";
    return metadata;
}
...
test "reads nikon make tagged tiff plane" {
    ...
    const make = "Nikon Corporation\x00";
    ...
    try data.appendSlice(std.testing.allocator, &.{ 33, 44 });

    try std.testing.expect(matches(data.items));
    const metadata = try readMetadata(data.items);
    try std.testing.expectEqualStrings("nikon", metadata.format);
    try std.testing.expectEqual(@as(u32, 2), metadata.width);

    const plane = try readPlane(std.testing.allocator, data.items);
    defer std.testing.allocator.free(plane.data);
    try std.testing.expectEqualStrings("nikon", plane.metadata.format);
    try std.testing.expectEqualSlices(u8, &.{ 33, 44 }, plane.data);
    ...
    try std.testing.expectEqualStrings("nikon", region_plane.metadata.format);
    try std.testing.expectEqualSlices(u8, &.{44}, region_plane.data);
}

test "matches tiff ep standard tag without make" {
    var data: std.ArrayList(u8) = .empty;
    defer data.deinit(std.testing.allocator);
