...
pub fn isPath(path: []const u8) bool {
    return hasExtension(path, "apl") or hasExtension(path, "tnb") or hasExtension(path, "mtb") or hasExtension(path, "tif") or hasExtension(path, "tiff");
}
...
fn selectedTiffPath(allocator: std.mem.Allocator, io: std.Io, path: []const u8) ![]u8 {
    if (isTiffPath(path)) {
        const apl = try findAplPath(allocator, io, path);
        allocator.free(apl);
        return allocator.dupe(u8, path);
    }
    const apl = try findAplPath(allocator, io, path);
    defer allocator.free(apl);
    const root = try parentPath(allocator, apl);
    defer allocator.free(root);
    return findFirstTiffRecursive(allocator, io, root, 0);
}

fn findAplPath(allocator: std.mem.Allocator, io: std.Io, path: []const u8) ![]u8 {
    if (hasExtension(path, "apl")) return allocator.dupe(u8, path);
    var current = try parentPath(allocator, path);
    defer allocator.free(current);
    var depth: usize = 0;
    while (depth < 4) : (depth += 1) {
        if (try firstSiblingApl(allocator, io, current)) |candidate| return candidate;
...
fn firstSiblingApl(allocator: std.mem.Allocator, io: std.Io, dir_path: []const u8) !?[]u8 {
    var dir = std.Io.Dir.cwd().openDir(io, dir_path, .{ .iterate = true }) catch return null;
    defer dir.close(io);
    var iter = dir.iterate();
    while (try iter.next(io)) |entry| {
        if (entry.kind == .file and hasExtension(entry.name, "apl")) {
...
fn findFirstTiffRecursive(allocator: std.mem.Allocator, io: std.Io, root: []const u8, depth: usize) ![]u8 {
    if (depth > 6) return error.FileNotFound;
    ...
    while (try iter.next(io)) |entry| {
        ...
        if (entry.kind == .directory and first_dir == null) {
...
test "reads apl dataset through first nested tiff" {
    const root = "apl-test";
    const nested = "apl-test/sample_DocumentFiles/series";
    const apl_path = "apl-test/sample.apl";
    const tiff_path = "apl-test/sample_DocumentFiles/series/image.tif";
    std.Io.Dir.cwd().deleteFile(std.testing.io, tiff_path) catch {};
    std.Io.Dir.cwd().deleteFile(std.testing.io, apl_path) catch {};
    std.Io.Dir.cwd().deleteDir(std.testing.io, nested) catch {};
    std.Io.Dir.cwd().deleteDir(std.testing.io, "apl-test/sample_DocumentFiles") catch {};
    std.Io.Dir.cwd().deleteDir(std.testing.io, root) catch {};
    try std.Io.Dir.cwd().createDir(std.testing.io, root, .default_dir);
    try std.Io.Dir.cwd().createDir(std.testing.io, "apl-test/sample_DocumentFiles", .default_dir);
    try std.Io.Dir.cwd().createDir(std.testing.io, nested, .default_dir);
    defer std.Io.Dir.cwd().deleteDir(std.testing.io, nested) catch {};
    defer std.Io.Dir.cwd().deleteDir(std.testing.io, "apl-test/sample_DocumentFiles") catch {};
    defer std.Io.Dir.cwd().deleteDir(std.testing.io, root) catch {};
    try std.Io.Dir.cwd().writeFile(std.testing.io, .{ .sub_path = apl_path, .data = "APL" });
    defer std.Io.Dir.cwd().deleteFile(std.testing.io, apl_path) catch {};
    var tiff_data: std.ArrayList(u8) = .empty;
    defer tiff_data.deinit(std.testing.allocator);
    try appendTinyTiff(&tiff_data, 66);
    ...
    const metadata = try readMetadataPath(std.testing.allocator, std.testing.io, apl_path);
    try std.testing.expectEqualStrings("apl", metadata.format);
    ...
    try std.testing.expectEqualStrings("apl", plane.metadata.format);
    try std.testing.expectEqualSlices(u8, &.{66}, plane.data);
}
