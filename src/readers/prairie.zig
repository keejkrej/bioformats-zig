...
const prairie_tag_1 = 33628;
const prairie_tag_2 = 33629;
const prairie_tag_3 = 33630;

pub fn matches(data: []const u8) bool {
    const software = tiff.firstIfdAsciiTag(data, software_tag) orelse return false;
    if (std.mem.indexOf(u8, software, "Prairie") == null) return false;
    return tiff.firstIfdContainsTag(data, prairie_tag_1) and
        tiff.firstIfdContainsTag(data, prairie_tag_2) and
        tiff.firstIfdContainsTag(data, prairie_tag_3);
}

pub fn readMetadata(data: []const u8) bio.ReaderError!bio.Metadata {
    var metadata = try tiff.readMetadata(data);
    metadata.format = "prairie";
    return metadata;
}
...
test "reads prairie tagged tiff plane" {
    ...
    try std.testing.expectEqualStrings("prairie", metadata.format);
    try std.testing.expectEqual(@as(u32, 2), metadata.width);

    const plane = try readPlane(std.testing.allocator, data.items);
    defer std.testing.allocator.free(plane.data);
    try std.testing.expectEqualStrings("prairie", plane.metadata.format);
    ...
    try std.testing.expectEqualStrings("prairie", region_plane.metadata.format);
    try std.testing.expectEqualSlices(u8, &.{92}, region_plane.data);
}

test "rejects prairie private tags without prairie software" {
    var data: std.ArrayList(u8) = .empty;
    defer data.deinit(std.testing.allocator);
