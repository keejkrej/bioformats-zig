...
pub fn readPlaneIndex(allocator: std.mem.Allocator, data: []const u8, plane_index: u32) bio.ReaderError!bio.Plane {
    ...
    plane.metadata.format = "simplepci";
    return plane;
}
...
test "reads simplepci tagged tiff plane" {
    ...
    try appendEntry(&data, 270, 2, description.len, ifd_end);
    ...
    try std.testing.expectEqualStrings("simplepci", metadata.format);

    const plane = try readPlane(std.testing.allocator, data.items);
    defer std.testing.allocator.free(plane.data);
    try std.testing.expectEqualStrings("simplepci", plane.metadata.format);
