...
const software_prefix = "IonpathMIBI";
...
pub fn readPlaneIndex(allocator: std.mem.Allocator, data: []const u8, plane_index: u32) bio.ReaderError!bio.Plane {
    ...
    plane.metadata.format = "ionpathmibi";
    return plane;
}
...
test "reads ionpath mibi tagged tiff plane" {
    ...
    const software = software_prefix ++ " 1.0\x00";
    ...
    try appendEntry(&data, 305, 2, software.len, ifd_end);
    try appendU32Le(&data, 0);
    try data.appendSlice(std.testing.allocator, software);
    try data.append(std.testing.allocator, 67);

    try std.testing.expect(matches(data.items));
    const metadata = try readMetadata(data.items);
    try std.testing.expectEqualStrings("ionpathmibi", metadata.format);

    const plane = try readPlane(std.testing.allocator, data.items);
    defer std.testing.allocator.free(plane.data);
    try std.testing.expectEqualStrings("ionpathmibi", plane.metadata.format);
