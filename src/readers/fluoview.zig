...
const mmheader_tag = 34361;
const mmstamp_tag = 34362;

pub fn matches(data: []const u8) bool {
    const comment = tiff.firstIfdAsciiTag(data, image_description_tag) orelse "";
    return (std.mem.indexOf(u8, comment, "FLUOVIEW") != null and tiff.firstIfdContainsTag(data, mmheader_tag)) or
        tiff.firstIfdContainsTag(data, mmstamp_tag) or
        std.mem.startsWith(u8, comment, "Andor");
}

pub fn readMetadata(data: []const u8) bio.ReaderError!bio.Metadata {
    var metadata = try tiff.readMetadata(data);
    metadata.format = "fluoview";
    return metadata;
}
...
pub fn readPlaneIndex(allocator: std.mem.Allocator, data: []const u8, plane_index: u32) bio.ReaderError!bio.Plane {
    ...
    plane.metadata.format = "fluoview";
    return plane;
}
...
test "reads fluoview tagged tiff plane" {
    ...
    try appendEntry(&data, mmheader_tag, 4, 1, 1);
    try appendU32Le(&data, 0);
    try data.appendSlice(std.testing.allocator, comment);
    try data.append(std.testing.allocator, 73);

    try std.testing.expect(matches(data.items));
    const metadata = try readMetadata(data.items);
    try std.testing.expectEqualStrings("fluoview", metadata.format);

    const plane = try readPlane(std.testing.allocator, data.items);
    defer std.testing.allocator.free(plane.data);
    try std.testing.expectEqualStrings("fluoview", plane.metadata.format);
    try std.testing.expectEqualSlices(u8, &.{73}, plane.data);
}

test "reads abd tiff identified by andor comment" {
    var data: std.ArrayList(u8) = .empty;
    defer data.deinit(std.testing.allocator);
    ...
    const ifd_end = 8 + 2 + entry_count * 12 + 4;
    const comment = "Andor Bio-imaging Division\x00";
    const comment_offset = ifd_end;
    const pixel_offset = comment_offset + comment.len;
