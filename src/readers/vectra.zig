...
test "reads vectra qptiff plane" {
    ...
    try data.append(std.testing.allocator, 223);

    try std.testing.expect(matches(data.items));
    const metadata = try readMetadata(data.items);
    try std.testing.expectEqualStrings("vectra", metadata.format);

    const plane = try readPlane(std.testing.allocator, data.items);
    defer std.testing.allocator.free(plane.data);
    try std.testing.expectEqualStrings("vectra", plane.metadata.format);
