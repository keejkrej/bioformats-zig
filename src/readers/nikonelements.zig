...
const nikon_xml_tag = 65332;

pub fn matches(data: []const u8) bool {
    return tiff.containsTag(data, nikon_xml_tag);
}

pub fn readMetadata(data: []const u8) bio.ReaderError!bio.Metadata {
    var metadata = try tiff.readMetadata(data);
    metadata.format = "nikonelements";
    return metadata;
}
...
pub fn readPlaneIndex(allocator: std.mem.Allocator, data: []const u8, plane_index: u32) bio.ReaderError!bio.Plane {
    ...
    plane.metadata.format = "nikonelements";
    return plane;
}
...
test "reads nikon elements tagged tiff plane" {
    const data = [_]u8{
        'I', 'I', 42, 0, 8, 0, 0,   0,
        10,  0,   0,  1, 4, 0, 1,   0,
        0,   0,   1,  0, 0, 0, 1,   1,
    ...
    try std.testing.expectEqualStrings("nikonelements", metadata.format);

    const plane = try readPlane(std.testing.allocator, &data);
    defer std.testing.allocator.free(plane.data);
    try std.testing.expectEqualStrings("nikonelements", plane.metadata.format);
