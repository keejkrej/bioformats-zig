...
pub fn matches(data: []const u8) bool {
    ...
    return std.mem.startsWith(u8, software, "eaZYX") or
        std.mem.startsWith(u8, software, "SCIL_Image") or
        std.mem.startsWith(u8, software, "IDL");
}

pub fn readMetadata(data: []const u8) bio.ReaderError!bio.Metadata {
    var metadata = try tiff.readMetadata(data);
    metadata.format = "mias";
    return metadata;
}
...
test "reads mias tagged tiff plane" {
    ...
    try std.testing.expectEqualStrings("mias", metadata.format);

    const plane = try readPlane(std.testing.allocator, data.items);
    defer std.testing.allocator.free(plane.data);
    try std.testing.expectEqualStrings("mias", plane.metadata.format);
