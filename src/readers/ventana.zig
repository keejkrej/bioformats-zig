...
pub fn matches(data: []const u8) bool {
    ...
    return std.mem.indexOf(u8, xml, "iScan") != null;
}

pub fn readMetadata(data: []const u8) bio.ReaderError!bio.Metadata {
    var metadata = try tiff.readMetadata(data);
    metadata.format = "ventana";
    return metadata;
}
...
test "reads ventana tagged tiff plane" {
    ...
    try appendEntry(&data, xml_tag, 2, xml.len, @intCast(ifd_end));
    try appendU32Le(&data, 0);
    try data.appendSlice(std.testing.allocator, xml);
    try data.append(std.testing.allocator, 144);

    try std.testing.expect(matches(data.items));
    const metadata = try readMetadata(data.items);
    try std.testing.expectEqualStrings("ventana", metadata.format);

    const plane = try readPlane(std.testing.allocator, data.items);
    defer std.testing.allocator.free(plane.data);
    try std.testing.expectEqualStrings("ventana", plane.metadata.format);
