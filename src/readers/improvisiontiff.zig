...
pub fn readPlaneIndex(allocator: std.mem.Allocator, data: []const u8, plane_index: u32) bio.ReaderError!bio.Plane {
    ...
    plane.metadata.format = "improvisiontiff";
    return plane;
}
...
test "reads improvision tagged tiff plane" {
    ...
    try std.testing.expectEqualStrings("improvisiontiff", metadata.format);

    const plane = try readPlane(std.testing.allocator, data.items);
    defer std.testing.allocator.free(plane.data);
    try std.testing.expectEqualStrings("improvisiontiff", plane.metadata.format);
