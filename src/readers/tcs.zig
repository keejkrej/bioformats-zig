...
const document_name_tag = 269;
const software_tag = 305;

pub fn matches(data: []const u8) bool {
    if (tiff.firstIfdAsciiTag(data, document_name_tag)) |document| {
        if (std.mem.startsWith(u8, std.mem.trim(u8, document, " \t\r\n\x00"), "CHANNEL")) return true;
    }
    if (tiff.firstIfdAsciiTag(data, software_tag)) |software| {
        if (std.mem.startsWith(u8, std.mem.trim(u8, software, " \t\r\n\x00"), "TCS")) return true;
    }
    return false;
}
...
test "reads leica tcs tiff identified by software" {
    ...
    try data.append(std.testing.allocator, 88);

    try std.testing.expect(matches(data.items));
    const metadata = try readMetadata(data.items);
    try std.testing.expectEqualStrings("tcs", metadata.format);

    const plane = try readPlane(std.testing.allocator, data.items);
    defer std.testing.allocator.free(plane.data);
    try std.testing.expectEqualStrings("tcs", plane.metadata.format);
    try std.testing.expectEqualSlices(u8, &.{88}, plane.data);
}

test "reads leica tcs tiff identified by channel document name" {
    var data: std.ArrayList(u8) = .empty;
    defer data.deinit(std.testing.allocator);
    ...
    const entry_count = 10;
    const ifd_end = 8 + 2 + entry_count * 12 + 4;
    const document = "CHANNEL 1 INDEX 1 2001:01:02 03:04:05 FORMAT TIFF\x00";
    const document_offset = ifd_end;
    const pixel_offset = document_offset + document.len;
    ...
    try appendEntry(&data, 278, 4, 1, 1);
    try appendEntry(&data, 279, 4, 1, 1);
    try appendEntry(&data, document_name_tag, 2, document.len, @intCast(document_offset));
    try appendU32Le(&data, 0);
    try data.appendSlice(std.testing.allocator, document);
    try data.append(std.testing.allocator, 34);

    try std.testing.expect(matches(data.items));
    const plane = try readPlane(std.testing.allocator, data.items);
    defer std.testing.allocator.free(plane.data);
    try std.testing.expectEqualSlices(u8, &.{34}, plane.data);
}
