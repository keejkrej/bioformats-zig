...
const magic = "SlideBook";
const slidebook_tags = [_]u16{ 65000, 65001, 65002, 65004, 65007, 65005 };

pub fn matches(data: []const u8) bool {
    const software = tiff.firstIfdAsciiTag(data, software_tag) orelse return false;
    if (!std.mem.eql(u8, software, magic)) return false;
    if (tiff.firstIfdAsciiTag(data, image_description_tag)) |comment| {
        if (comment.len != 0) return false;
    }
    for (slidebook_tags) |tag| {
        if (tiff.firstIfdContainsTag(data, tag)) return true;
    }
    return false;
}
...
pub fn readPlaneIndex(allocator: std.mem.Allocator, data: []const u8, plane_index: u32) bio.ReaderError!bio.Plane {
    ...
    plane.metadata.format = "slidebooktiff";
    return plane;
}
...
test "reads slidebook tiff tagged plane" {
    ...
    try appendEntry(&data, 65004, 2, 2, 0);
    try appendU32Le(&data, 0);
    try data.appendSlice(std.testing.allocator, software);
    try data.append(std.testing.allocator, 91);

    try std.testing.expect(matches(data.items));
    const metadata = try readMetadata(data.items);
    try std.testing.expectEqualStrings("slidebooktiff", metadata.format);

    const plane = try readPlane(std.testing.allocator, data.items);
    defer std.testing.allocator.free(plane.data);
    try std.testing.expectEqualStrings("slidebooktiff", plane.metadata.format);
    try std.testing.expectEqualSlices(u8, &.{91}, plane.data);
}

test "rejects slidebook tiff with non-empty comment" {
    var data: std.ArrayList(u8) = .empty;
    defer data.deinit(std.testing.allocator);
    ...
    const ifd_end = 8 + 2 + entry_count * 12 + 4;
    const software = magic ++ "\x00";
    const comment = "not empty\x00";
    const software_offset = ifd_end;
    const comment_offset = software_offset + software.len;
    ...
    try appendEntry(&data, 279, 4, 1, 1);
    try appendEntry(&data, software_tag, 2, software.len, @intCast(software_offset));
    try appendEntry(&data, image_description_tag, 2, comment.len, @intCast(comment_offset));
    try appendEntry(&data, 65004, 2, 2, 0);
    try appendU32Le(&data, 0);
    try data.appendSlice(std.testing.allocator, software);
    try data.appendSlice(std.testing.allocator, comment);
