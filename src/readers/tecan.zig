...
pub fn isPath(path: []const u8) bool {
    if (hasExtension(path, "db")) return true;
    if (hasExtension(path, "tif") or hasExtension(path, "tiff")) return pathHasComponent(path, "Images");
    return false;
}
...
test "does not claim unrelated tiff paths" {
    ...
    try std.testing.expect(isPath("tecan-test/Images/WellA1/img.tif"));
}
