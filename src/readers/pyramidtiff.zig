...
pub fn matches(data: []const u8) bool {
    ...
    return std.mem.indexOf(u8, software, "Faas") != null;
}

pub fn readMetadata(data: []const u8) bio.ReaderError!bio.Metadata {
    var metadata = try tiff.readMetadata(data);
    metadata.format = "pyramidtiff";
    return metadata;
}
...
pub fn readPlaneIndex(allocator: std.mem.Allocator, data: []const u8, plane_index: u32) bio.ReaderError!bio.Plane {
    ...
    plane.metadata.format = "pyramidtiff";
    return plane;
}
...
test "reads pyramid tiff tagged plane" {
    ...
    try data.append(std.testing.allocator, 123);

    try std.testing.expect(matches(data.items));
    const metadata = try readMetadata(data.items);
    try std.testing.expectEqualStrings("pyramidtiff", metadata.format);

    const plane = try readPlane(std.testing.allocator, data.items);
    defer std.testing.allocator.free(plane.data);
    try std.testing.expectEqualStrings("pyramidtiff", plane.metadata.format);
